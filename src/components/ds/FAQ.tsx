...
export function FAQ() {
  const left = FAQ_CLIENTS.filter((i) => i.cat === "left");
  const right = FAQ_CLIENTS.filter((i) => i.cat === "right");

  return (
    <section id="faq" className="relative scroll-mt-20 lg:scroll-mt-24">
      <div className="mx-auto max-w-[1400px] px-6 pb-12 pt-8 lg:pb-16 lg:pt-10">
        <Reveal>
          <div className="mb-10">
        ...
        <Reveal delay={120}>
          <div className="grid grid-cols-1 gap-y-10 lg:grid-cols-2 lg:gap-x-12 lg:gap-y-0">
            <FaqColumn eyebrow="Le service & son fonctionnement" items={left} />
            <FaqColumn eyebrow="Délais, confiance & données" items={right} />
          </div>
        </Reveal>
      </div>
    </section>
