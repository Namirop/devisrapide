...
export async function assignLeadToPros(input: {
  ...
  for (const pro of pros) {
    ...
    if (finalStatus === "PENDING" && assignmentId) {
      // Wording client C : titre + corps avec urgence et extrait projet.
    ...
    // ── Push notification "auto-accept declenche" (client G).
    ...
    //    achat" V1 : client F est email-seulement (email LeadAccepted
    ...
    if (
      ...
      // Email I — client : pendant email du push, opt-in (respecte
